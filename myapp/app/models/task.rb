class Task < ApplicationRecord
  ...
  scope :all_sort_by, ->(target, sort_type) { includes(:labels).order({ "#{target}": sort_type }) }
  ...
  scope :extract_matched_label, ->(label_id) { where(labels: { id: label_id }) if label_id.present? }
